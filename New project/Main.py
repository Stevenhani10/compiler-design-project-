...
class Main:
        ...
        for Line in range(0 , len(self.Become_Syntax_Check)):
            for Index in range(0, len(self.Become_Syntax_Check[Line])):
                ...
                if self.Become_Syntax_Check[Line][Index][1] == "Become":
                    ...
                    elif self.Become_Syntax_Check[Line][Index + 1][1] not in ["Variable", "Integer", "Boolean", "String", "Float", "Error","Open Bracket"] and self.Become_Syntax_Check[Line][Index + 1][0] != "-":
                                    ...
                                    for Variable_Index2 in range(0, len(self.Declared_Variables)):
                                        if self.Become_Syntax_Check[Line][Index + 1][0] == self.Declared_Variables[Variable_Index2][0]:
                                            if self.Declared_Variables[Variable_Index][1] != self.Declared_Variables[Variable_Index2][1]:
                                                self.Become_Syntax_Check[Line][Index + 1][0] = f"Error: Line {Line + 1} You can't assign {self.Declared_Variables[Variable_Index2][0]} ({self.Declared_Variables[Variable_Index2][1]}) to a varaible ({self.Declared_Variables[Variable_Index][0]}) of type {self.Declared_Variables[Variable_Index][1]}"
                                                self.Become_Syntax_Check[Line][Index + 1][1] = "Error"
                                            flag_found = True
                                            break
                                    if flag_found:
    ...
    def Save_History(self):
        ...
        for Line in range(0 , len(self.Final_List)):
            for Index in range(0, len(self.Final_List[Line])):
                if self.Final_List[Line][Index][1] == "Become":
                    if (self.Final_List[Line][Index - 1][1] == "Variable") and ((self.Final_List[Line][Index + 1][1] in ["Variable", "Integer", "Boolean", "String", "Float","Open Bracket"]) or (self.Final_List[Line][Index + 1][0] == "-")):
                        Flag_Save = True
                        self.History.append([self.Final_List[Line][Index - 1][0], []])
                elif Flag_Save:
                    if self.Final_List[Line][Index][1] != "Semicolon" and self.Final_List[Line][Index][1] != "Comma":
                        self.History[len(self.History) - 1][1].append(self.Final_List[Line][Index][0])
                    else:
                        Flag_Save = False
    ...
    def calculate_history(self):
        self.results = []  # Create an empty list to store the results
        self.variables = []
        for item in self.History:
            ...
            for i in range(len(item[1])):
                flag_var=False
                flag_val=False
                for k in reversed(range(len(self.variables))):
                    if item[1][i] == self.variables[k]:
                        flag_var=True
                        if  self.results[k][1] != None:
                            flag_val=True
                            item[1][i] = str(self.results[k][1])  # Convert float to string
                            break
                if flag_val==False:
                    break
            if (flag_val==True and flag_var==True) or flag_var==False:
                expression = ''.join(str(elem) for elem in item[1])  # Concatenate elements as strings
                result = eval(expression)
                self.results.append([variable, result])  # Store the result in the list
                self.variables.append(variable)
            else:
                self.results.append(['Error','Variable has no assigned value '])  # Store the result in the list
                self.variables.append('Variable Not Found')
        print('The result:', self.results)
...
"bool" : "Identifiers", "char" : "Identifiers", "+" : "Operator","**":"Operator", "-" : "Operator", "/" : "Operator", 
